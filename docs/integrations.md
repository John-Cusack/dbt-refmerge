...
The pre-commit hook, the GitHub Action and `scan --format github` need dbt-refmerge 0.2.0 or later. Column-pruning leads need 0.3.0 or later.
...
The repository ships a [pre-commit](https://pre-commit.com) hook, `dbt-refmerge-scan`. It runs `dbt-refmerge scan --fail-on finding` whenever a `.sql` file changes, and fails when any model has duplicate imports or imports that can select fewer columns.

```yaml
# .pre-commit-config.yaml
repos:
  - repo: https://github.com/John-Cusack/dbt-refmerge
    rev: v0.3.0 # a release tag including column-pruning leads
    hooks:
...
The repository is also a composite action. It annotates pull requests at each duplicate-import or column-pruning lead:
...
jobs:
  scan:
    ...
    steps:
      ...
      - uses: John-Cusack/dbt-refmerge@v0.3.0 # pin a release tag, or a commit SHA
        with:
...
| `package` | `dbt-refmerge` | The pip requirement to install, e.g. `dbt-refmerge==0.3.0`. |
